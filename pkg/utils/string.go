...
func BuildWhereInPrepareParameter(length int) string {
	return strings.TrimRight(strings.Repeat("?,", length), ",")
}

func Uint64Contain(dst uint64, lst ...uint64) bool {
	for _, s := range lst {
		if s == dst {
			return true
		}
...
func Uint64sContainsAny(source []uint64, dest ...uint64) bool {
	for _, b := range dest {
		for _, a := range source {
			if b == a {
				return true
